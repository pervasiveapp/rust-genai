...
/// Obtain a Google Cloud Bearer token (OAuth2 access token) from, in order:
/// 0) a ready token in `GOOGLE_VERTEX_TOKEN`,
/// 1) a base64/raw service-account JSON in `env_var_name` (default `GCP_SERVICE_ACCOUNT_B64`),
/// 2) ADC via `GOOGLE_APPLICATION_CREDENTIALS` (service-account file),
/// 3) `gcloud auth application-default print-access-token`.
///
/// Returns `Ok(None)` when no credential source is available, so callers can fall back to an
/// adapter default. Exposed so consumers can compose a per-adapter auth resolver (e.g. mint a
/// Vertex token only for the Vertex adapter kind while other kinds use their own auth).
pub async fn fetch_google_access_token(
	env_var_name: Option<&str>,
	scopes: Option<&[&str]>,
) -> Result<Option<AuthData>, Error> {
	let env_name = env_var_name.unwrap_or(GCP_SA_B64_ENV);
	...
	if let Ok(path) = std::env::var("GOOGLE_APPLICATION_CREDENTIALS") {
		let json = std::fs::read_to_string(path).map_err(|e| Error::Custom(format!("GCP ADC read file: {e}")))?;
		let account = gcp_auth::CustomServiceAccount::from_json(&json)
...
pub fn google_adc_or_sa_b64_auth_resolver(env_var_name: Option<&str>, scopes: Option<&[&str]>) -> AuthResolver {
	...
	AuthResolver::from_resolver_async_fn(move |_model: ModelIden| {
		...
		Box::pin(async move {
			let scope_refs: Vec<&str> = scopes.iter().map(|s| s.as_str()).collect();
			fetch_google_access_token(Some(&env_name), Some(&scope_refs)).await
